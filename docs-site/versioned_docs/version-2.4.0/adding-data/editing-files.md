...
In addition to the built-in system snippets above, you can define your own reusable transaction templates for quick expansion:
*   **Stand-alone Snippets File**: User-defined templates are loaded from a dedicated `snippets.beancount` file stored in your root structured folder layout. When you toggle the feature on in **Settings → BQL → Editor Settings**, this file is automatically generated with a `sampleSnippet` template if it is missing.
*   **Autocompletion Trigger**: Start typing the name of a custom snippet at the beginning of any empty line in a `.beancount` file to trigger the autocomplete suggestion popup. Selecting a snippet will populate it instantly.
*   **Real-time Cache Reloading**: The plugin watches `snippets.beancount` for changes. Any updates or new templates saved to the file are automatically loaded in memory without requiring a plugin reload or Obsidian restart.
*   **Command Integration**: You can open the command palette (`Ctrl/Cmd + P`) and execute the **"Open Beancount snippets file"** command to quickly open your snippets file for editing.
